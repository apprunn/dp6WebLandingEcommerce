<template>
	<div class="page-detail-product">
		...
		<div v-if="disabled" class="product-disabled">
			<h1 :style="`color:${globalColors.primary}`" class="sorry-text">
				Lo sentimos, este producto está desactivado :(
			</h1>
		</div>
		<div v-else class="detail-product-top">
			...
			<product-detail
				...
				:data="productDetails"
				:warehouses-total="warehousesTotal"
		...
		<div>
			<product-related :relateds="relateds" v-if="relateds.length" />
		</div>
...
async function loadProduct() {
	try {
		...
		this.warehousesTotal = this.product.warehouseProductDetails;
		const isExist = this.warehousesTotal.some(
			w => w.id === this.product.warehouseProduct.warehouseId,
		);
		if (!isExist) {
			this.warehousesTotal.push(this.product.warehouseProduct);
		}
		...
		if (error.data.message === 'PRODUCT_NOT_FOUND') {
			...
			this.$store.dispatch('LOAD_RELATED_PRODUCTS', {
				context: this,
				id: this.id,
			});
		} else {
...
async function loadData(id) {
	...
	if (this.getCommerceData.settings.flagGrouper !== 2) {
		...
		[{ data: this.childrens }] = await Promise.all(requests);
	}
	...
	this.allFeatures = this.childrens.reduce(
		(acum, children) => acum.concat(children.features),
		[],
	);
	this.features = this.allFeatures.reduce((acum, feature) => {
		const index = acum.findIndex(a => a.name === feature.name);
		if (index > -1) {
			const indexValue = acum[index].values.findIndex(
				i => i.name === feature.value,
			);
			if (indexValue === -1) {
				const value = {
					name: feature.value,
...
function possibleFeature(possibles) {
	this.features = this.features.map((element, index) => {
		element.values.forEach((value, indexValue) => {
			if (this.productsFilter.length === 1) {
				...
				this.$set(
					this.features[index].values[indexValue],
					'active',
					!!flag.length,
				);
				...
				const flag = possibles.filter(
					p => p.value === value.code && !value.active,
				);
				this.$set(
					this.features[index].values[indexValue],
					'possible',
	...
	if (this.productsFilter.length === 1) {
		this.productsFilter[0].images = this.productsFilter[0].images.map(
			(i, index) => {
				const newImage = { ...i };
				newImage.select = index === 0;
				return newImage;
...
async function loadOpinions() {
	...
	const { data: response } = process.env.PRODUCTS_READ_REPORT
		? await this.$httpProductsReadPublic.get('question-answer/public', {
				params,
		  })
		: await this.$httpProductsPublic.get('question-answer/public', { params });
...
function inputQuantity(num) {
	...
	if (validQuantity) {
		...
		this.showNotification(
			`Cantidad: ${num} no disponible inputQuantity`,
			'primary',
		);
	}
}
...
async function openDialog() {
	...
	try {
		const { data: response } = await this.$httpProductsPublic.get(
			`products-public/${id}/stock-by-warehouse`,
		);
...
function selectedUnit(unit) {
	...
	if (this.quantityStock > this.product.stockWarehouse) {
		const validQuantity = parseInt(
			this.product.stockWarehouse / unit.quantity,
			10,
		);
		const newProductdetail = { ...this.product };
...
export default {
	...
	computed: {
		...mapGetters(['getCommerceData', 'getPromotionalBanner', 'token']),
