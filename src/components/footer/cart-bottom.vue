<template>
	<div>
		<nav
			@click="goShopping"
			v-if="(getTotalItems > 0 && !indeterminate) || iscar"
			:style="activeStyle"
			class="nav"
		>
			<div :style="activeStyle" class="nav-content-cart">
				...
				<div class="summary-info">
					<span>Total</span>
					<h2 class="total-sumary">
						{{ getCurrencySymbol }}
						{{ iscar ? getTotalPrice : getTotalToBuy | currencyFormat }}
					</h2>
				</div>
				<div v-if="iscar" class="car-action">
					<button @click="onClickEvent($event, 'less')" class="btn-add-cart">
						<span :style="colorBackGroundStyle" class="txt-icon-min"></span>
					</button>
					<input
						@keypress="isNumber($event)"
						@click="$event.stopPropagation()"
						class="cube txt-quantity"
						v-model="quantityInput"
					/>
					<button @click="onClickEvent($event, 'more')" class="btn-add-cart">
						<span :style="colorTextStyle" class="txt-icon-plus"> + </span>
					</button>
					<button
